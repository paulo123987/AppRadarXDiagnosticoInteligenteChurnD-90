...
def get_top_journey_patterns(df_classified: pd.DataFrame, top_n: int = 3) -> list:
    ...
    """
    if df_classified is None or df_classified.empty or "MACRO_MOTIVO" not in df_classified.columns:
        return []
    
    # Agrupar por cliente e criar lista ordenada de motivos
    journeys = df_classified.sort_values(["ID_CLIENTE", "DATETIME_TRANSCRICAO_LIGACAO"]).groupby("ID_CLIENTE")["MACRO_MOTIVO"].apply(lambda x: " → ".join(x))
